...
$sql_mail = "SELECT email,washdate,washtime from tblcarwashbooking where id='$id'";
$query_mail = $dbh -> prepare($sql_mail);
$query_mail->execute();
$result_mail=$query_mail->fetchAll(PDO::FETCH_ASSOC);
if($query_mail->rowCount() > 0)
{
	$res = $result_mail[0];
	$approveEmail=$res['email'];
	$washdate=$res['washdate'];
	$washtime=$res['washtime'];
}

$sql="update  tblcarwashbooking set status='Approved' where id=:id";
...
if(isset($_POST['approve']))
{
	$mailer = new Mailer(
		$approveEmail,
		"Booking Successful",
		"Your booking for car wash at auto hub is confirmed. Washing date: " . $washdate . " and time: " . $washtime
	);
...
$sql_mail = "SELECT email from tblcarwashbooking where id='$id'";
$query_mail = $dbh -> prepare($sql_mail);
$query_mail->execute();
$result_mail=$query_mail->fetchAll(PDO::FETCH_ASSOC);
if($query_mail->rowCount() > 0)
...
$sql="update  tblcarwashbooking set status='Rejected' where id=:id";
$query = $dbh->prepare($sql);
/*$query->bindParam(':ttype',$ttype,PDO::PARAM_STR);
$query->bindParam(':transactionno',$transactionno,PDO::PARAM_STR);
$query->bindParam(':message',$message,PDO::PARAM_STR);*/
...
if(isset($_POST['reject']))
{
	$mailer = new Mailer($approveEmail,"Booking is not Successfull","plese select another time slot");
	$mailer->send();
}
...
if($ptype==1): echo "BASIC CLEANING (₹100)";endif;
if($ptype==2): echo "PREMIUM CLEANING (₹250)";endif;
if($ptype==3): echo "COMPLEX CLEANING (₹500)";endif;
...
<?php if($result->adminRemark=='' && $result->status != '' && $result->status != 'Approved' && $result->status != 'Rejected'): ?>
    <td colspan="3">
        <form method="post">   
            <label><input type="submit" class="btn btn-custom" name="approve" value="approve"></label>
            <label><input type="submit" class="btn btn-custom" name="reject" value="reject"></label>
        </form>
    </td>
<?php endif; ?>

<?php if ($result->status == 'Approved' || $result->status == 'Rejected') : ?>
<?php endif; ?>
